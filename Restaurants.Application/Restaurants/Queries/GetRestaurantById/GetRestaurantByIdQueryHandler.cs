    ...
    IRestaurantRepository restaurantRepository): IRequestHandler<GetRestaurantByIdQuery, RestaurantDto?>
{
    public async Task<RestaurantDto> Handle(GetRestaurantByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        return mapper.Map<RestaurantDto>(await restaurantRepository.GetByIdAsync(request.Id) ?? throw new NotFoundException($"Restaurant with {request.Id} doesn't exist"));
    }
}
